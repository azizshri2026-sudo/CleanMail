...
actual class SmtpClient actual constructor(private val account: Account) {
    ...
    actual suspend fun sendMessage(message: OutgoingMessage): SmtpResult<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val allRecipients = (message.to + message.cc + message.bcc)
                .map { InternetAddress(it.address, it.name) }
                .toTypedArray()

            transport!!.sendMessage(mime, allRecipients)
        }.toSmtpResult()
    }
    ...
    actual suspend fun testConnection(): SmtpResult<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            connect().let {
                if (it is SmtpResult.Failure) throw Exception(it.error.toString())
            }
            disconnect()
        }.toSmtpResult()
    }
    ...
    private fun smtpProtocol() = when (account.smtpSecurity) {
        SecurityType.SSL_TLS -> "smtps"
        else -> "smtp"
    }
    ...
    private fun buildProperties(): Properties = Properties().apply {
        ...
        // Отключаем IPv6 если он недоступен
    ...
    private fun MimePart.setBodyContent(msg: OutgoingMessage) {
        when {
            msg.bodyHtml.isNotEmpty() && msg.bodyText.isNotEmpty() -> {
                ...
                val textPart = MimeBodyPart().apply { setText(msg.bodyText, "UTF-8", "plain") }
                val htmlPart = MimeBodyPart().apply { setText(msg.bodyHtml, "UTF-8", "html") }
